...
function getNewCharCode(code, shift) {
    if (code <= 90 && code >=65) {   //checks if original code is from a capital letter
        if (code + shift > 90) {    //if the shift is going to take it above the alphabet range
        ...
        } else if (code + shift < 65) { //if the shift is going to take it below the alphabet range
    ...
    } else if (code >= 97 && code <= 122) {   //checks if original code is from a lower case letter, and performs same process as above for upper case letters.
        if (code + shift > 122) {
